...
/// A trait used internally by Rust-Postgres to convert types into Postgres values.
...
impl<T: ToSql> ToSqlChecked for T {
    fn to_sql_checked(
        &self,
        ty: &Type,
        out: &mut BytesMut,
...
impl sealed::Sealed for &dyn ToSqlChecked {}

impl BorrowToSqlChecked for &dyn ToSqlChecked {
    #[inline]
    fn borrow_to_sql(&self) -> &dyn ToSqlChecked {
        *self
    }
...
impl sealed::Sealed for Box<dyn ToSqlChecked + Sync> {}

impl BorrowToSqlChecked for Box<dyn ToSqlChecked + Sync> {
    #[inline]
    fn borrow_to_sql(&self) -> &dyn ToSqlChecked {
        self.as_ref()
    }
...
impl sealed::Sealed for &(dyn ToSqlChecked + Sync) {}
...
impl BorrowToSqlChecked for &(dyn ToSqlChecked + Sync) {
    #[inline]
    fn borrow_to_sql(&self) -> &dyn ToSqlChecked {
        *self
    }
...
impl<T> sealed::Sealed for T where T: ToSqlChecked {}

impl<T> BorrowToSqlChecked for T
where
    T: ToSqlChecked,
{
    #[inline]
